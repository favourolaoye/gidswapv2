...
export function DesktopNav({ navLinks, activeLink, onLinkClick }: DesktopNavProps) {
  ...
  return (
    <nav className="hidden sticky top-0 z-50 bg-white dark:bg-[#1a1d29] md:flex items-center justify-between px-6 py-4 border-b border-gray-800">
          ...
          {navLinks.map((link) => (
            <Button
              ...
              className={`flex items-center gap-2 text-gray-600 dark:text-white hover:text-blue-600 dark:hover:text-blue-400 bg-transparent ${
                activeLink === link.name ? "border-2 border-blue-600 text-blue-600 rounded-full" : ""
      ...
      <div className="flex items-center gap-3">
        ...
        >
          {theme === "dark" ? <Sun className="w-5 h-5 text-gray-100"/> : <Moon className="w-5 h-5 text-gray-800" />}
        </Button>
