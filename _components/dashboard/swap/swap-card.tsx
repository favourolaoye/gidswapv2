...
export function SwapCard({
    ...
    const fetchCurrencies = async () => {
      ...
      try {
        const response = await axios.get(
          `${api}/api/fixfloat/trade/currencies`,
          {
        ...
        if (response.data.code === 0) {
          ...
          if (data.length > 1) {
            ...
            setReceiveCurrency(
              eth ||
                data.find((c) => c.coin !== (btc?.coin || data[0].coin)) ||
                data[1]
            );
          }
        }
  ...
  return (
    <div className="w-full max-w-md mx-auto space-y-4">
      <div className="bg-[#2a2d3a] rounded-xl p-4 ">
      ...
      <div className="bg-[#2a2d3a] rounded-2xl p-6 mb-6 space-y-2">
        {/* Sell Section */}
        <div className="mb-4">
          <SwapSection
            type="sell"
            ...
            usdValue={
              sellAmount && rate
                ? `$${(parseFloat(sellAmount) * parseFloat(rate)).toFixed(2)}`
                : undefined
            }
      ...
      {/* Swap Action Button */}
      <Button
        className="w-full futuristic-button bg-blue-700 hover:bg-blue-900 font-semibold py-4 rounded-xl"
        onClick={onSwap}
      >
        Swap
      </Button>
