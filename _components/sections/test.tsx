...
interface Currency {
  name: string;
  logo: string;
  rate: number; // Conversion rate relative to USDC
}

const currencies: Currency[] = [
  { name: "USDC", logo: "/images/usdc-logo.svg", rate: 1 },
  { name: "ETH", logo: "/images/eth-logo.svg", rate: 0.0005 }, // Example rates
  { name: "BTC", logo: "/images/btc-logo.svg", rate: 0.00002 },
];
...
const SwapForm: React.FC<{
  ...
  return (
    <form className="grid gap-3 text-sm font-orbitron text-gray-700 transition-all dark:text-white">
      <div className="relative rounded-[20px] bg-gray-50/50 p-3 dark:bg-white/5 backdrop-blur-sm border border-[#0d6fde]/20 dark:border-blue-400/20 shadow-md">
        ...
        <div className="flex flex-col space-y-2 rounded-xl bg-white px-3 py-2 dark:bg-neutral-900 shadow-sm">
          ...
          <div className="flex items-center justify-between gap-2">
            <input
              id={`amount-sent-${tab}`}
              inputMode="decimal"
              className="w-full rounded-lg border-b border-transparent bg-transparent py-1 text-2xl outline-none placeholder:text-gray-400 focus:border-[#0d6fde] dark:focus:border-blue-400 focus:outline-none text-neutral-900 dark:text-white/80"
              value={sendAmount}
        ...
        <div className="flex flex-col space-y-2 rounded-xl bg-white px-3 py-2 dark:bg-neutral-900 shadow-sm">
          ...
          <div className="flex items-center justify-between gap-2">
            ...
            >
              <Image
                src={receiveCurrency.logo || "/images/default-logo.svg"}
...
export default function Hero() {
  ...
  return (
    <div
      id="hero"
      className="flex min-h-screen w-full flex-col items-center justify-center overflow-y-auto py-16 bg-gradient-to-b from-black/10 to-transparent dark:from-black/50 dark:to-transparent"
    >
      <div className="flex flex-1 flex-col justify-center">
        <AnimatedSection className="w-full">
          <section className="w-full px-5 mb-8">
            <h1 className="flex flex-col items-center gap-1 text-center font-orbitron">
              <span className="text-2xl text-gray-600 dark:text-white/80 sm:text-3xl md:text-4xl">
                Buy, Sell, or Swap
              </span>
              <span className="text-xl font-bold sm:text-2xl md:text-3xl italic text-[#0d6fde] dark:text-blue-400">
                Crypto Instantly
              </span>
            </h1>
          </section>
        ...
        <div className="flex flex-col gap-2 mt-8">
          <p className="text-xs text-gray-400 dark:text-white/30 animate-bounce">
            Learn More
          </p>
          <ChevronDown className="w-4 h-4 mx-auto text-gray-300 dark:text-white/50 animate-bounce" />
        </div>
      </div>
